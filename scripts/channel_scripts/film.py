...
def _film_prompt(source_text: str, destination: str, *, length_instruction: str, layout_instruction: str) -> str:
    return (
        "تو یک نویسنده حرفه‌ای محتوای فیلم و سینما فارسی هستی.\n"
        ...
        "5) لحن خبری/تحلیلی سینمایی داشته باشد، نه رباتی.\n"
        ...
        "اگر متن ورودی خالی بود، فقط با اتکا به تصاویر/مدیای ورودی یک کپشن سینمایی مرتبط تولید کن."
    )
...
def main() -> None:
    ...
    try:
        ...
        if "ai_output_not_acceptable" in lowered:
            ...
            raise SystemExit(2)
        print(f"[film.py] non-fatal error: {exc}", file=sys.stderr)
        try:
            payload = _load_payload(Path(args.payload))
