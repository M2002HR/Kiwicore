...
PROMPT_LEAK_RE = re.compile(
    r"(professional persian film content writer|raw text|captions|ready-to-publish|rules|translation|rewrite)",
    re.IGNORECASE,
)
...
def _ai_mandatory_mode() -> bool:
    ...
    return forced and _film_ai_enabled()
...
def _source_tone_instruction(payload: dict) -> str:
    return (
        "لحن خروجی را همیشه خودمانی، راحت و محاوره‌ای نگه دار؛ "
        "از ساختارهای خشک و رسمی پرهیز کن و متن را طبیعی و روان بنویس."
    )
...
def _film_prompt(source_text: str, destination: str, *, length_instruction: str, layout_instruction: str) -> str:
    return (
        "تو یک نویسنده محتوای فیلم و سینما به فارسی محاوره‌ای هستی.\n"
        "ورودی می‌تواند متن خام، کپشن، یا تصاویر فیلم و سینما باشد.\n"
        "یک پست فارسی طبیعی، روان، خودمانی و آماده انتشار بنویس.\n\n"
        "قوانین قطعی:\n"
        "1) ترجمه تحت‌اللفظی ممنوع؛ مفهوم را طبیعی و محاوره‌ای بازنویسی کن.\n"
        ...
        "5) لحن سینماییِ صمیمی داشته باشد، نه رسمی، نه رباتی.\n"
        ...
        "اگر متن ورودی خالی بود، فقط با اتکا به تصاویر/مدیای ورودی یک کپشن سینمایی مرتبط و خودمانی تولید کن."
    )
...
def _generate_film_text(*, payload: dict, input_dir: Path, base_messages: list[dict]) -> str | None:
    ...
    if (
        ...
        refine_prompt = (
            "متن زیر را به یک پست فیلم و سینما فارسی روان، طبیعی و آماده انتشار بازنویسی کن.\n"
            ...
            f"{tone_instruction}\n\n"
            f"متن:\n{cleaned or source_text or 'از روی تصویر یک کپشن سینمایی بساز'}"
        )
    ...
    if not strict_sources:
        strict_sources.append("از روی تصویر، فقط یک کپشن سینمایی فارسی کوتاه و حرفه‌ای تولید کن.")
...
def build_messages(payload: dict, *, input_dir: Path) -> list[dict]:
    ...
    # If source has no caption/text, do not generate any new caption.
    # Just passthrough media as-is (guard has already decided relevance).
    ...
    generated = _generate_film_text(payload=payload, input_dir=input_dir, base_messages=base)
...
def main() -> None:
    ...
    try:
        ...
        if "ai_output_not_acceptable" in lowered:
            ...
            raise SystemExit(2)
        print(f"[film_informal.py] non-fatal error: {exc}", file=sys.stderr)
        try:
            payload = _load_payload(Path(args.payload))
