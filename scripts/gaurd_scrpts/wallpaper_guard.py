...
def _source_usernames(payload: dict) -> set[str]:
    message = payload.get("message") or {}
    route = payload.get("route") or {}
    out: set[str] = set()
    if isinstance(message, dict):
        normalized = _normalize_username(message.get("source_channel_username"))
        if normalized:
            out.add(normalized)
    if isinstance(route, dict):
        normalized = _normalize_username(route.get("source_channel_username"))
        if normalized:
            out.add(normalized)
    return out


def _has_nonempty_caption(payload: dict) -> bool:
    message = payload.get("message") or {}
    caption = str(message.get("caption") or "").strip()
    return bool(caption)


def _extract_mentioned_usernames(text: str) -> set[str]:
    matches = re.findall(r"(^|[\s(])@([a-zA-Z0-9_]{3,})\b", str(text or ""))
    return {str(handle).strip().lower() for _, handle in matches if str(handle).strip()}


def _extract_tme_usernames(text: str) -> set[str]:
    matches = re.findall(r"(?:https?://)?(?:t\.me|telegram\.me)/([a-zA-Z0-9_]{3,})\b", str(text or ""), flags=re.IGNORECASE)
    return {str(handle).strip().lower() for handle in matches if str(handle).strip()}


def _is_same_source_join_invite(payload: dict) -> bool:
    message = payload.get("message") or {}
    text = str(message.get("text") or "")
    caption = str(message.get("caption") or "")
    combined_raw = f"{text}\n{caption}"
    combined = _normalize_ad_text(combined_raw)
    if not combined.strip():
        return False

    source_usernames = _source_usernames(payload)
    ...
    if not any(token in combined for token in invite_tokens):
        return False

    mentioned = _extract_mentioned_usernames(combined_raw)
    linked = _extract_tme_usernames(combined_raw)
    all_targets = set(mentioned) | set(linked)
    if not all_targets:
        return False

    # Allow only self-channel invitations; invites to other channels remain blocked.
    return all_targets.issubset(source_usernames)
...
def _build_safety_request(payload: dict, input_dir: Path, model: str | None) -> dict:
    message = payload.get("message") or {}
...
def _build_wallpaper_relevance_request(payload: dict, input_dir: Path, model: str | None) -> dict:
    message = payload.get("message") or {}
    text = str(message.get("text") or "").strip()
    caption = str(message.get("caption") or "").strip()
    ...
    media_hint = ", ".join(media_kinds[:20]) if media_kinds else "<none>"

    user_text = (
        "Task: Decide if a Telegram post should be forwarded to a WALLPAPER destination channel.\n"
        "Return 1 (allow) only if content is genuinely wallpaper-oriented.\n"
        "Return 0 (block) for unrelated content, ads, promotions, engagement bait, or invites to other channels/pages.\n\n"
        "Block examples:\n"
        "- News/sports/politics/general entertainment text\n"
        "- Join/follow/subscribe/DM requests (with or without links)\n"
        "- Promotions/affiliate/referral/sponsored/shop/trading/casino/vpn posts\n"
        "- Captions like 'click here', 'use link below', 'add my instagram', etc.\n\n"
        "Allow examples:\n"
        "- Wallpaper captions (style, quality, resolution, wallpaper hashtags)\n"
        "- Minimal wallpaper metadata (e.g., #wallpaper #amoled #4k)\n"
        "- Empty/short caption when media itself appears to be wallpaper.\n\n"
        "Important:\n"
        "- Do not block only because of one ambiguous hashtag/word.\n"
        "- Use overall context from text + caption + attached media.\n\n"
        "Output strictly one character only: 1 or 0.\n\n"
        f"Media kinds:\n{media_hint}\n\n"
        f"Message text:\n{text or '<empty>'}\n\n"
        f"Message caption:\n{caption or '<empty>'}\n"
...
def _looks_like_wallpaper_post_for_tech_fail_open(payload: dict) -> bool:
    message = payload.get("message") or {}
    text = str(message.get("text") or "")
    caption = str(message.get("caption") or "")
    ...
    if any(token in combined for token in wallpaper_keywords):
        return True
    ...
    # On AI technical failures, allow only clearly image-only posts.
    return image_count > 0 and non_image_count == 0
...
def _is_irrelevant_wallpaper_caption(payload: dict) -> bool:
    if _is_same_source_join_invite(payload):
        return False
    ...
    has_link = bool(re.search(r"(https?://|t\.me/|telegram\.me/|instagram\.com/|youtu\.be/|youtube\.com/|x\.com/)", combined))
    social_cta = (
        "join us",
        "add my instagram",
        "follow me",
    ...
    if any(token in combined for token in social_cta):
        # Social/engagement CTA is irrelevant for wallpaper forwarding, even without explicit URL.
        return True

    linkless_engagement_bait = (
        "click here",
        "tap here",
        "wanna laugh",
        "want to laugh",
    ...
    if any(token in combined for token in linkless_engagement_bait):
        return True

    strong_unrelated_phrases = (
        "رسمی شد",
        "لیگ برتر",
        "champions league",
        "قهرمان",
    ...
    if any(token in combined for token in strong_unrelated_phrases):
        return True

    unrelated_topic_signals = (
        "football",
        "soccer",
        "match",
        "goal",
    ...
    topic_hits = sum(1 for token in unrelated_topic_signals if token in combined)
    if topic_hits >= 2:
        return True
    if topic_hits >= 1 and has_link:
        return True
    return False
...
def main() -> None:
    ...
    wallpaper_ai_enabled = os.getenv("GUARD_WALLPAPER_AI_ENABLED", "true").strip().lower() in {"1", "true", "yes", "on"}
    wallpaper_ai_fail_open = os.getenv("GUARD_WALLPAPER_AI_FAIL_OPEN", "false").strip().lower() in {"1", "true", "yes", "on"}
    wallpaper_ai_tech_fail_soft_allow = (
        os.getenv("GUARD_WALLPAPER_AI_TECH_FAIL_SOFT_ALLOW", "true").strip().lower() in {"1", "true", "yes", "on"}
    )
    skip_ai_when_no_caption = os.getenv("GUARD_SKIP_AI_WHEN_NO_CAPTION", "true").strip().lower() in {"1", "true", "yes", "on"}
    ...
    if _is_irrelevant_wallpaper_caption(payload):
        print("false: irrelevant_wallpaper_caption")
        return
    ...
    if skip_ai_when_no_caption and not _has_nonempty_caption(payload):
        print("true")
        return

    if wallpaper_ai_enabled:
        # Same-source channel join invite is allowed by policy and should not be rejected by AI relevance.
        if not _is_same_source_join_invite(payload):
            relevance_body = _build_wallpaper_relevance_request(payload, Path(args.input_dir), model=model)
            is_relevant, relevance_reason = _call_guard_api(
                endpoint=endpoint,
                body=relevance_body,
                timeout_sec=timeout_sec,
                fail_open=wallpaper_ai_fail_open,
            )
            if not is_relevant:
                if (
                    wallpaper_ai_tech_fail_soft_allow
                    and _is_ai_technical_failure_reason(relevance_reason)
                    and _looks_like_wallpaper_post_for_tech_fail_open(payload)
                ):
                    pass
                else:
                    print(f"false: wallpaper_relevance:{relevance_reason}")
                    return

    body = _build_safety_request(payload, Path(args.input_dir), model=model)
