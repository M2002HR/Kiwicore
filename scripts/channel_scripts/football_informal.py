...
def _source_tone_instruction(payload: dict) -> str:
    ...
    if not source:
        return (
            "لحن خروجی را ساده، روان و نیمه‌خودمانی نگه دار؛ "
            "نه خشک و رسمی، نه شلوغ و پراغراق."
        )
    ...
    if casual_score >= formal_score + 2:
        return (
            "لحن خروجی را خودمانی و روان نگه دار؛ "
            "صمیمی باشد اما از شوخی/اسلنگِ زیاد و بی‌ربط پرهیز کن."
        )
    if formal_score >= casual_score + 1:
        return (
            "اگر ورودی رسمی یا خبری است، خروجی را فقط کمی نرم‌تر و طبیعی‌تر کن؛ "
            "متن را شلوغ نکن و از محاوره‌ی افراطی دوری کن."
        )
    return (
        "لحن خروجی را نیمه‌خودمانی و متعادل نگه دار؛ "
        "هم روان و دوستانه باشد، هم جمع‌وجور و حرفه‌ای."
    )
...
def _football_prompt(source_text: str, destination: str, *, length_instruction: str, layout_instruction: str) -> str:
    return (
        "تو یک نویسنده محتوای فوتبالی به فارسی محاوره‌ای هستی.\n"
        ...
        "5) لحن فوتبالیِ طبیعی و متناسب با ورودی داشته باشد؛ "
        "اگر ورودی رسمی بود، خروجی را فقط کمی خودمانی کن و شلوغ نکن.\n"
        ...
        "9.5) از اغراق، اسلنگ بیش از حد، یا لحن نمایشی بی‌دلیل پرهیز کن.\n\n"
        f"10) {length_instruction}\n"
        f"11) {layout_instruction}\n\n"
        f"امضای مقصد: {destination or '<none>'}\n\n"
        f"ورودی:\n{source_text or '<empty>'}\n\n"
        "اگر متن ورودی خالی بود، فقط با اتکا به تصاویر/مدیای ورودی یک کپشن فوتبالی مرتبط و خودمانی تولید کن."
    )
...
def main() -> None:
    ...
    try:
        ...
        if "ai_output_not_acceptable" in lowered:
            ...
            raise SystemExit(2)
        print(f"[football_informal.py] non-fatal error: {exc}", file=sys.stderr)
        try:
            payload = _load_payload(Path(args.payload))
