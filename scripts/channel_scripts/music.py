...
PROMPT_LEAK_RE = re.compile(
    r"(professional persian music content writer|raw text|captions|ready-to-publish|rules|translation|rewrite)",
    re.IGNORECASE,
)
...
EMOJI_VARIATION = "\ufe0f"
EMOJI_KEYCAP = "\u20e3"
AUDIO_FILE_EXTS = (".mp3", ".m4a", ".aac", ".wav", ".ogg", ".opus", ".flac")
DEMO_HINT_RE = re.compile(r"(demo|preview|teaser|snippet|دمو|پیش[\s‌-]?نمایش|نمونه)", re.IGNORECASE)
FULL_TRACK_HINT_RE = re.compile(
    r"(full\s*version|original|main\s*track|دانلود|نسخه\s*اصلی|آهنگ\s*کامل|موزیک\s*کامل|320|128)",
    re.IGNORECASE,
)
METADATA_UNKNOWN_RE = re.compile(
    r"^(unknown|untitled|audio|song|track(\s*\d+)?|new\s*song|بدون\s*نام|نامشخص|ناشناس|نامعلوم|ناموجود)$",
    re.IGNORECASE,
)
MUSIC_BLACKLIST_ARTIST_GROUPS: tuple[tuple[str, ...], ...] = (
    ("Toomaj Salehi", "توماج صالحی", "توماج"),
    ("Fadaei", "فدایی", "فدائي"),
    ("Shapur", "شاپور"),
    ("Hichkas", "هیچکس", "هيچکس"),
    ("Bahram", "Bahram Nouraei", "بهرام", "بهرام نورایی", "بهرام نورائي"),
    ("Saman Yasin", "Saman Yaseen", "سامان یاسین", "سامان ياسين"),
)
_MUSIC_META_AI_CACHE: dict[str, dict[str, str]] = {}
...
def _ai_mandatory_mode() -> bool:
    ...
    return forced and _music_ai_enabled()


def _input_index_by_local_name(payload: dict) -> dict[str, dict]:
    out: dict[str, dict] = {}
    ...
    for item in inputs:
        if not isinstance(item, dict):
            continue
        local_name = _resolve_local_name(item)
        if not local_name or local_name in out:
            continue
        out[local_name] = item
    return out


def _normalize_persian_letters(value: str) -> str:
    text = str(value or "")
    return (
        text.replace("ي", "ی")
        .replace("ك", "ک")
...
def _clean_music_meta_value(value: object) -> str:
    if isinstance(value, list):
        merged = ", ".join(str(part).strip() for part in value if str(part).strip())
    else:
        merged = str(value or "").strip()
    if not merged:
        return ""
    text = _normalize_multiline_text(_normalize_persian_letters(merged))
...
def _has_name_chars(value: str) -> bool:
    return bool(re.search(r"[A-Za-z\u0600-\u06FF]", str(value or "")))


def _compact_name_key(value: str) -> str:
    text = _normalize_persian_letters(str(value or "")).lower()
    return re.sub(r"[^0-9a-z\u0600-\u06FF]+", "", text)


def _split_artist_candidates(value: str) -> list[str]:
    text = str(value or "").strip()
    if not text:
        return []
    parts = re.split(r"\s*(?:,|/|\\|\||&| and | feat\.?| ft\.?| x | × | و | با )\s*", text, flags=re.IGNORECASE)
    out: list[str] = []
    for part in parts:
        cleaned = _clean_music_meta_value(part)
        if cleaned:
            out.append(cleaned)
    return out or ([_clean_music_meta_value(text)] if _clean_music_meta_value(text) else [])


def _normalize_artist_for_match(value: str) -> tuple[str, set[str], str]:
    normalized = _normalize_persian_letters(str(value or "")).lower()
    normalized = re.sub(r"[^0-9a-z\u0600-\u06FF]+", " ", normalized)
    normalized = re.sub(r"\s+", " ", normalized).strip()
    tokens = {tok for tok in normalized.split(" ") if tok}
    compact = _compact_name_key(normalized)
    return normalized, tokens, compact


def _is_blacklisted_artist(artist_name: str) -> bool:
    if not artist_name:
        return False

    artist_candidates = _split_artist_candidates(artist_name)
    if not artist_candidates:
        artist_candidates = [artist_name]

    for candidate in artist_candidates:
        _, candidate_tokens, candidate_compact = _normalize_artist_for_match(candidate)
        for alias_group in MUSIC_BLACKLIST_ARTIST_GROUPS:
            for alias in alias_group:
                alias_norm, alias_tokens, alias_compact = _normalize_artist_for_match(alias)
                if not alias_norm:
                    continue
                if len(alias_tokens) == 1:
                    ...
                    if token in candidate_tokens:
                        return True
                    if len(token) >= 5 and token in candidate_compact:
                        return True
                    continue
                if alias_tokens and alias_tokens.issubset(candidate_tokens):
                    return True
                if alias_compact and alias_compact in candidate_compact:
                    return True
    return False


def _parse_title_artist_from_file_name(file_name: str) -> tuple[str | None, str | None]:
    ...
    if len(parts) != 2:
        return (None, None)
    left = _clean_music_meta_value(parts[0])
    right = _clean_music_meta_value(parts[1])
    if not left or not right:
        return (None, None)
    return (right, left)


def _is_valid_music_meta_piece(value: str) -> bool:
    text = _clean_music_meta_value(value)
    if not text:
        return False
    if not _has_name_chars(text):
        return False
    if METADATA_UNKNOWN_RE.match(text):
        return False
    if len(_compact_name_key(text)) < 2:
        return False
    return True


def _extract_music_track_meta(item: dict) -> dict[str, str] | None:
    title_candidates = [
        item.get("title"),
    ...
    for value in title_candidates:
        cleaned = _clean_music_meta_value(value)
        if cleaned:
            title = cleaned
            break
    artist = ""
    for value in artist_candidates:
        cleaned = _clean_music_meta_value(value)
        if cleaned:
            artist = cleaned
            break
    ...
    if not title or not artist:
        parsed_title, parsed_artist = _parse_title_artist_from_file_name(str(item.get("file_name") or item.get("local_name") or ""))
        if not title and parsed_title:
            title = parsed_title
        if not artist and parsed_artist:
            artist = parsed_artist

    if not _is_valid_music_meta_piece(title) or not _is_valid_music_meta_piece(artist):
        return None
    if _compact_name_key(title) == _compact_name_key(artist):
        return None

    local_name = str(item.get("local_name") or item.get("file_name") or "").strip()
    return {
        "title": title,
        "artist": artist,
...
def _is_audio_like_input_item(item: dict) -> bool:
    ...
    return _is_audio_like_name(file_name)


def _validate_music_metadata(payload: dict) -> tuple[bool, list[dict[str, str]]]:
    inputs = payload.get("inputs")
    if not isinstance(inputs, list):
        return (True, [])
    ...
    for item in inputs:
        if not isinstance(item, dict):
            continue
        if not _is_audio_like_input_item(item):
            continue

        track = _extract_music_track_meta(item)
        if track is None:
            return (False, [])
        if _is_blacklisted_artist(track["artist"]):
            return (False, [])
        tracks.append(track)
    return (True, tracks)


def _music_identity_block(tracks: list[dict[str, str]]) -> str | None:
    if not tracks:
        return None
    first = tracks[0]
    title = _clean_music_meta_value(first.get("title"))
    artist = _clean_music_meta_value(first.get("artist"))
    if not title or not artist:
        return None
    return f"🎵 {title}\n🎤 {artist}"


def _persian_fallback_text(value: str) -> str:
    text = _clean_music_meta_value(value)
    if not text:
        return ""
    if PERSIAN_CHAR_RE.search(text):
        return text
    # For Latin-only names, do not do heuristic transliteration here.
    # Persian form must come from AI to avoid broken stage-name rendering.
    return ""


def _latin_to_persian_approx(value: str) -> str:
    text = str(value or "")
    if not text:
        return ""
    # Lightweight transliteration for display-only fallback.
    # Keeps route stable without additional network/AI dependency.
    multi = [
        ("tch", "چ"),
        ("ch", "چ"),
        ("sh", "ش"),
    ...
    while i < len(text):
        ...
        if hit:
            continue
        out.append(single.get(lower[i], text[i]))
        i += 1
    translit = "".join(out)
...
def _extract_json_object(text: str) -> dict | None:
    raw = _strip_md_fence(str(text or ""))
    if not raw:
        return None
    try:
        obj = json.loads(raw)
...
def _clean_music_lyric_line(value: object) -> str:
    text = _normalize_multiline_text(str(value or ""))
    if not text:
        return ""
    ...
    if not line:
        return ""
    if len(line) < 6 or len(line) > 84:
        return ""
    if not PERSIAN_CHAR_RE.search(line):
        return ""
    if URL_RE.search(line) or MENTION_RE.search(line) or HASHTAG_RE.search(line):
        return ""
    if _is_promotional_text(line):
        return ""
    latin_tokens = len(LATIN_WORD_RE.findall(line))
    token_count = max(1, len(re.findall(r"\S+", line)))
    if (latin_tokens / token_count) > 0.2:
        return ""
    return line


def _smart_music_meta(title: str, artist: str) -> dict[str, str]:
    base_title_fa = _persian_fallback_text(title)
    base_artist_fa = _persian_fallback_text(artist)
    result = {"title_fa": base_title_fa, "artist_fa": base_artist_fa, "lyric": ""}
    if not _music_ai_enabled():
        return result
    endpoint = _pick_ai_endpoint()
    if not endpoint:
        return result
    cache_key = f"{_compact_name_key(title)}::{_compact_name_key(artist)}"
    cached = _MUSIC_META_AI_CACHE.get(cache_key)
    if isinstance(cached, dict):
        return {
            "title_fa": str(cached.get("title_fa") or base_title_fa),
            "artist_fa": str(cached.get("artist_fa") or base_artist_fa),
            "lyric": str(cached.get("lyric") or ""),
        }

    prompt = (
        "فقط یک JSON معتبر برگردان و هیچ متن اضافه نده.\n"
        "هدف: فارسی‌سازی حرفه‌ای نام آهنگ و نام خواننده بدون خراب‌کردن لقب/استیج‌نیم.\n"
        "اگر نام فارسی رایج دارد همان را بده؛ اگر مطمئن نیستی رشته خالی بده.\n"
        "اصطلاحات موسیقی را ترجمه نکن و همان لاتین نگه دار (مثل feat., ft., remix, remastered, live, acoustic, EP, LP, Vol., Pt.).\n"
        "در title_fa فقط بخش‌های قابل فارسی‌سازی را فارسی کن و اصطلاحات/تگ‌های موسیقی را دست‌نخورده نگه دار.\n"
        "در artist_fa فقط اسم هنرمند/هنرمندها را با املای درست فارسی بده و القاب را خراب نکن.\n"
        "فقط وقتی مصرع معروف و واقعی این آهنگ را با اطمینان بالا می‌دانی، یک مصرع کوتاه بده؛ "
        "در غیر اینصورت lyric را خالی بگذار.\n"
        "فرمت خروجی:\n"
        "{\"title_fa\":\"...\",\"artist_fa\":\"...\",\"lyric\":\"...\"}\n\n"
        f"title: {title}\n"
        f"artist: {artist}\n"
    )
    parts = [{"text": prompt}]
    ...
    if model:
        body["model"] = model
    raw = _call_gemini_text(endpoint=endpoint, body=body, timeout_sec=min(8.0, _ai_timeout_sec()))
    obj = _extract_json_object(raw or "")
    if isinstance(obj, dict):
        title_fa = _guard_music_terms_in_fa(source=title, fa_candidate=_clean_music_meta_value(obj.get("title_fa")))
        artist_fa = _guard_music_terms_in_fa(source=artist, fa_candidate=_clean_music_meta_value(obj.get("artist_fa")))
        lyric = _clean_music_lyric_line(obj.get("lyric"))
        if title_fa and PERSIAN_CHAR_RE.search(title_fa):
            result["title_fa"] = title_fa
        if artist_fa and PERSIAN_CHAR_RE.search(artist_fa):
            result["artist_fa"] = artist_fa
        if lyric:
            result["lyric"] = lyric
    _MUSIC_META_AI_CACHE[cache_key] = dict(result)
    return result


def _music_term_tokens(text: str) -> set[str]:
    raw = str(text or "")
    if not raw:
        return set()
    patterns = [
    ...
    for pat in patterns:
        for m in re.finditer(pat, lowered, flags=re.IGNORECASE):
            token = lowered[m.start():m.end()].strip()
            if token:
                out.add(token.replace(" ", ""))
    return out


def _guard_music_terms_in_fa(*, source: str, fa_candidate: str) -> str:
    candidate = _clean_music_meta_value(fa_candidate)
    if not candidate:
        return ""
    src_terms = _music_term_tokens(source)
    if not src_terms:
        return candidate
    cand_flat = re.sub(r"\s+", "", candidate.lower())
    for term in src_terms:
        if term and term not in cand_flat:
            return ""
    return candidate


def _is_identity_only_block_text(text: str) -> bool:
    cleaned = _normalize_multiline_text(str(text or ""))
    if not cleaned:
        return False
    lines = [line.strip() for line in cleaned.splitlines() if line.strip()]
    if len(lines) < 2:
        return False
    title_line = next((line for line in lines if line.startswith("🎵 ") and not line.startswith("🎵 فارسی:")), "")
    artist_line = next((line for line in lines if line.startswith("🎤 ")), "")
    if not title_line or not artist_line:
        return False
    title = _clean_music_meta_value(title_line[2:])
    artist = _clean_music_meta_value(artist_line[2:])
    if not _is_valid_music_meta_piece(title) or not _is_valid_music_meta_piece(artist):
        return False
    if _is_blacklisted_artist(artist):
        return False
    return True


def _merge_identity_and_text(identity: str, text: str) -> str:
    body = _normalize_multiline_text(text)
    if not body:
        return identity
    if identity in body:
        return body
    return _normalize_multiline_text(f"{identity}\n\n{body}")


def _inject_music_identity(messages: list[dict], tracks: list[dict[str, str]]) -> list[dict]:
    identity = _music_identity_block(tracks)
    if not identity:
        return messages

    out = [dict(item) for item in messages if isinstance(item, dict)]
    ...
    for idx, item in enumerate(out):
        ...
        if msg_type == "text" and first_text_idx is None:
            first_text_idx = idx
        if msg_type in CAPTION_TYPES:
            if first_media_idx is None:
                first_media_idx = idx
            caption = str(item.get("caption") or "").strip()
            if caption and first_caption_idx is None:
                first_caption_idx = idx

    if first_caption_idx is not None:
        existing = str(out[first_caption_idx].get("caption") or "").strip()
        out[first_caption_idx]["caption"] = _merge_identity_and_text(identity, existing)
        return out

    if first_text_idx is not None:
        existing = str(out[first_text_idx].get("text") or "").strip()
        out[first_text_idx]["text"] = _merge_identity_and_text(identity, existing)
        return out

    if first_media_idx is not None:
        out[first_media_idx]["caption"] = identity
        return out

    return [{"type": "text", "text": identity}, *out]


def _sanitize_file_piece(value: str) -> str:
    text = _normalize_persian_letters(str(value or ""))
    text = re.sub(r"[\\/:*?\"<>|]+", " ", text)
    text = re.sub(r"\s+", " ", text).strip(" .-_")
    return text


def _build_pretty_audio_file_name(*, artist: str, title: str, source_name: str) -> str:
    safe_artist = _sanitize_file_piece(artist) or "Unknown Artist"
    safe_title = _sanitize_file_piece(title) or "Unknown Title"
    suffix = str(Path(source_name or "").suffix or "").strip().lower()
    if suffix not in AUDIO_FILE_EXTS:
        suffix = ".mp3"
    name = f"{safe_artist} - {safe_title}{suffix}"
    if len(name) <= 150:
        return name
    stem = f"{safe_artist} - {safe_title}"
    keep = max(20, 150 - len(suffix))
    return f"{stem[:keep].rstrip(' .-_')}{suffix}"


def _resolve_existing_media_path(*, rel_path: str, input_dir: Path, output_dir: Path) -> Path | None:
...
def _materialize_alias_file(*, source_path: Path, alias_path: Path) -> bool:
    try:
        alias_path.parent.mkdir(parents=True, exist_ok=True)
        if alias_path.exists():
            return alias_path.is_file()
        try:
...
def _apply_pretty_audio_file_names(messages: list[dict], tracks: list[dict[str, str]], *, input_dir: Path, output_dir: Path) -> list[dict]:
    if not messages or not tracks:
        return messages

    track_by_local: dict[str, dict[str, str]] = {}
    for track in tracks:
        key = str(track.get("local_name") or "").strip()
        if key:
            track_by_local[key] = track
    ...
    for item in messages:
        ...
        rel_path = str(obj.get("path") or "").strip()
        track = track_by_local.get(rel_path)
        if not track:
            out.append(obj)
            continue

        source_path = _resolve_existing_media_path(rel_path=rel_path, input_dir=input_dir, output_dir=output_dir)
        if source_path is None:
            out.append(obj)
            continue

        alias_name = _build_pretty_audio_file_name(
            artist=str(track.get("artist") or ""),
            title=str(track.get("title") or ""),
            source_name=rel_path,
        )
        base_stem = Path(alias_name).stem
        suffix = Path(alias_name).suffix
        final_alias = alias_name
        counter = 2
        while final_alias in used_aliases or (output_dir / final_alias).exists():
            final_alias = f"{base_stem} ({counter}){suffix}"
            counter += 1

        alias_path = output_dir / final_alias
        if _materialize_alias_file(source_path=source_path, alias_path=alias_path):
            obj["path"] = final_alias
...
def _is_audio_like_name(name: str) -> bool:
    lowered = str(name or "").strip().lower()
    if not lowered:
        return False
    return lowered.endswith(AUDIO_FILE_EXTS)


def _media_sort_priority(message: dict, *, input_index: dict[str, dict]) -> int:
    msg_type = str(message.get("type") or "").strip().lower()
    path = str(message.get("path") or "").strip()
    input_item = input_index.get(path) or {}
    file_name = str(input_item.get("file_name") or path or "").strip()
    mime_type = str(input_item.get("mime_type") or "").strip().lower()
    hint_blob = f"{file_name}\n{path}".lower()

    if msg_type == "photo":
        return 0
    if msg_type in {"video", "animation", "video_note"}:
        return 1
    if msg_type == "voice":
        return 1
    if msg_type == "audio":
        if DEMO_HINT_RE.search(hint_blob) and not FULL_TRACK_HINT_RE.search(hint_blob):
            return 1
        return 2
    if msg_type == "document":
        if DEMO_HINT_RE.search(hint_blob) and not FULL_TRACK_HINT_RE.search(hint_blob):
            return 1
        if _is_audio_like_name(file_name) or _is_audio_like_name(path) or mime_type.startswith("audio/"):
            return 2
        return 3
    return 4


def _reorder_music_media_flow(messages: list[dict], *, payload: dict) -> list[dict]:
    if not isinstance(messages, list):
        return []

    sortable_types = {"photo", "video", "voice", "audio", "document", "animation", "video_note"}
    indexed_media: list[tuple[int, dict]] = []
    for idx, item in enumerate(messages):
        if not isinstance(item, dict):
            continue
        msg_type = str(item.get("type") or "").strip().lower()
        if msg_type in sortable_types:
            indexed_media.append((idx, item))
    if len(indexed_media) < 2:
        return [dict(item) for item in messages if isinstance(item, dict)]

    input_index = _input_index_by_local_name(payload)
    sorted_media = sorted(
        indexed_media,
        key=lambda pair: (_media_sort_priority(pair[1], input_index=input_index), pair[0]),
    )
    if [idx for idx, _ in sorted_media] == [idx for idx, _ in indexed_media]:
        return _normalize_first_media_caption([dict(item) for item in messages if isinstance(item, dict)])

    media_iter = iter([dict(item) for _, item in sorted_media])
    out: list[dict] = []
    for item in messages:
        if not isinstance(item, dict):
            continue
        msg_type = str(item.get("type") or "").strip().lower()
        if msg_type in sortable_types:
            out.append(next(media_iter))
            continue
        out.append(dict(item))
    return _normalize_first_media_caption(out)


def _normalize_first_media_caption(messages: list[dict]) -> list[dict]:
    first_media_idx: int | None = None
    first_caption_idx: int | None = None
    first_caption_value: str | None = None

    for idx, item in enumerate(messages):
        if not isinstance(item, dict):
            continue
        msg_type = str(item.get("type") or "").strip().lower()
        if msg_type not in CAPTION_TYPES:
            continue
        if first_media_idx is None:
            first_media_idx = idx
        caption = str(item.get("caption") or "").strip()
        if first_caption_idx is None and caption:
            first_caption_idx = idx
            first_caption_value = caption

    if (
        first_media_idx is None
        or first_caption_idx is None
        or first_caption_value is None
        or first_media_idx == first_caption_idx
    ):
        return messages

    out = [dict(item) if isinstance(item, dict) else item for item in messages]
    out[first_media_idx]["caption"] = first_caption_value
    out[first_caption_idx].pop("caption", None)
    return out
...
def _music_prompt(source_text: str, destination: str, *, length_instruction: str, layout_instruction: str) -> str:
    return (
        "تو یک نویسنده حرفه‌ای محتوای موزیکی فارسی هستی.\n"
        "ورودی می‌تواند متن خام، کپشن، یا مدیای مربوط به آهنگ باشد.\n"
        ...
        "5) لحن معرفی موزیک و آهنگ داشته باشد، نه رباتی.\n"
        ...
        "اگر متن ورودی خالی بود، فقط با اتکا به مدیای ورودی یک کپشن موزیکی مرتبط تولید کن."
    )
...
def _has_any_media_caption(messages: list[dict]) -> bool:
    for item in messages:
        if not isinstance(item, dict):
            continue
        msg_type = str(item.get("type") or "").strip().lower()
...
def _generate_music_text(*, payload: dict, input_dir: Path, base_messages: list[dict]) -> str | None:
    ...
    if (
        ...
        refine_prompt = (
            "متن زیر را به یک پست موزیکی فارسی روان، طبیعی و آماده انتشار بازنویسی کن.\n"
            ...
            f"{tone_instruction}\n\n"
            f"متن:\n{cleaned or source_text or 'از روی تصویر یک کپشن موزیکی بساز'}"
        )
    ...
    if not strict_sources:
        strict_sources.append("از روی تصویر، فقط یک کپشن موزیکی فارسی کوتاه و حرفه‌ای تولید کن.")
...
def build_messages(payload: dict, *, input_dir: Path, output_dir: Path | None = None) -> list[dict]:
    base = _reorder_music_media_flow(_build_base_messages(payload), payload=payload)
    ...
    meta_ok, tracks = _validate_music_metadata(payload)
    if not meta_ok:
        return []
    # Hard gate: this route must forward music only.
    # If no audio track is detected, block the message completely.
    if not tracks:
        return []
    ...
    # Music route policy: no AI rewrite/translation. Keep original media flow
    # and only inject verified English metadata identity.
    generated = None
    ai_mandatory = False
    ...
    if _has_media_without_caption(out) and not _has_any_media_caption(out) and not ai_mandatory and has_textual_source:
    ...
    out = _inject_music_identity(out, tracks)
    if output_dir is not None:
        out = _apply_pretty_audio_file_names(out, tracks, input_dir=input_dir, output_dir=output_dir)
        ...
        for item in out:
            ...
            if msg_type == "text":
                ...
                if text and not _is_persian_acceptable(text, destination=destination):
                    # Media-only updates can legitimately have title/artist metadata in Latin script.
                    if not (not has_textual_source and _is_identity_only_block_text(text)):
            ...
            if msg_type in CAPTION_TYPES:
                caption = str(obj.get("caption") or "").strip()
                if caption and not _is_persian_acceptable(caption, destination=destination):
                    if not (not has_textual_source and _is_identity_only_block_text(caption)):
                        raise RuntimeError("ai_output_not_acceptable")
                sanitized.append(obj)
                continue
            sanitized.append(obj)
...
def main() -> None:
    ...
    try:
        ...
        messages = build_messages(payload, input_dir=Path(args.input_dir), output_dir=Path(args.output_dir))
    except Exception as exc:
        lowered = str(exc).lower()
        if "ai_generation_required_failed" in lowered:
            print("[music.py] strict-skip: ai_generation_required_failed", file=sys.stderr)
            messages = []
        elif "ai_output_not_acceptable" in lowered:
            print("[music.py] strict-skip: ai_output_not_acceptable", file=sys.stderr)
            messages = []
        else:
            print(f"[music.py] non-fatal error: {exc}", file=sys.stderr)
            try:
                payload = _load_payload(Path(args.payload))
                messages = _safe_fail_open_messages(
                    _reorder_music_media_flow(_build_base_messages(payload), payload=payload),
                    destination=_destination_signature(payload),
                )
            except Exception:
                messages = []
