...
def _route_name(payload: dict) -> str:
    ...
    if not isinstance(route, dict):
        return ""
    return str(route.get("name") or "").strip()


def _channel_script_name(payload: dict) -> str:
    route = payload.get("route")
    if not isinstance(route, dict):
        return ""
    return str(route.get("channel_script") or "").strip().lower()


def _is_music_route(payload: dict) -> bool:
    name = _route_name(payload).lower()
    script_name = _channel_script_name(payload)
    return script_name == "music.py" or "music" in name


def _is_audio_input_item(item: dict) -> bool:
...
def _split_artist_like(value: str) -> list[str]:
    ...
    out: list[str] = []
    for p in parts:
        token = re.sub(r"\s+", " ", str(p or "").strip().lower())
        token = token.strip("()[]{}<>\"'`.,:;!?")
        if token:
            out.append(token)
    return out
...
def _is_unknown_artist_value(value: str) -> bool:
    parts = _split_artist_like(value)
    if not parts:
        return False
    return any(part in UNKNOWN_ARTIST_MARKERS for part in parts)


def _music_guard_violation(payload: dict) -> str | None:
    if not _is_music_route(payload):
        return None

    min_duration_sec = 60
    raw_min = str(os.getenv("GUARD_MUSIC_MIN_DURATION_SEC", "60") or "60").strip()
    try:
        min_duration_sec = max(1, int(float(raw_min)))
    except Exception:
        min_duration_sec = 60
    ...
    for item in inputs:
        if not isinstance(item, dict):
            continue
        if not _is_audio_input_item(item):
            continue
        ...
        if duration > 0 and duration < min_duration_sec:
            return f"music_too_short:{duration}s"
        ...
        for artist_value in artist_candidates:
            if _is_unknown_artist_value(str(artist_value or "")):
                return "music_unknown_artist"
    return None
...
def main() -> None:
    ...
    if block_ads and _is_obvious_advertisement(payload):
        ...
        return
    music_violation = _music_guard_violation(payload)
    if music_violation:
        print(f"false: {music_violation}")
        return
